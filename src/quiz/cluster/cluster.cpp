...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
	...
	viewer->setBackgroundColor (0, 0, 0);
  	...
  	viewer->addCube(window.x_min, window.x_max, window.y_min, window.y_max, 0, 0, 0, 0, 0, "window"); //Last 3 zeros are the color
  	return viewer;
}
...
void proximity(int id,const std::vector<std::vector<float>>& points, std::vector<int>& clusterP,std::vector<bool>& pointsProcessed, KdTree* tree, float distanceTol)
{

	// TODO: Fill out this function to return list of indices for each cluster

	pointsProcessed[id]=true;
	clusterP.push_back(id);
	std::vector<int> nearest = tree->search(points[id],distanceTol);


	for(int id : nearest){
		if(!pointsProcessed[id])
			proximity(id,points,clusterP,pointsProcessed,tree,distanceTol);
	}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	std::vector<bool> pointsProcessed(points.size(),false);

	int idP=0;

	while(idP<pointsProcessed.size()){
		if(!pointsProcessed[idP]){
			std::vector<int> clusterP;
			proximity(idP,points,clusterP,pointsProcessed,tree,distanceTol);
			clusters.push_back(clusterP);
		}
		idP++;
	}
